    (() => {
        ...
        const css = `
            ...
            .arq-input { 
                ...
                color: #fff; border-radius: 10px; margin-bottom: 15px; 
            ...
            .saved-targets-area { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 15px; }
            .saved-chip { 
                background: rgba(255,255,255,0.08); padding: 4px 10px; border-radius: 15px; 
                font-size: 10px; display: flex; align-items: center; cursor: pointer; color: #cbd5e1;
                transition: all 0.3s ${EASE_SMOOTH};
        ...
        const updateUI = () => {
            ...
            const items = (currentSubTab === 'me-not-back') ? list : [...list].reverse();
        ...
        const renderSavedTargets = () => {
            ...
            savedTargets.forEach((target, index) => {
                ...
                chip.innerHTML = `<span class="chip-name">${target}</span> <span class="chip-del">×</span>`;
                chip.onclick = () => { document.getElementById('stalker-target').value = target; runLogic('stalk'); };
                chip.querySelector('.chip-del').onclick = (e) => {
                    e.stopPropagation(); 
                    if(confirm(`Remover ${target}?`)) {
                        savedTargets = savedTargets.filter(t => t !== target);
        ...
        async function scanGraph(userId, type) {
            ...
            while(hasNext) {
                ...
                data.edges.forEach(e => {
                    list.push({ id: e.node.id, user: e.node.username, pic: e.node.profile_pic_url, follows_viewer: e.node.follows_viewer });
                });
        ...
        async function runLogic(mode) {
            ...
            try {
                ...
                const followingList = isMyProfile ? rawFollowing.filter(u => u.follows_viewer) : rawFollowing; 
